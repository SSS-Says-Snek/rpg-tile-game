        ...
        for entity, (pos, movement) in self.world.get_components(Position, Movement):
            ...
            for nested_entity, (nested_pos, nested_health) in self.world.get_components(
                ...
                if entity != self.player and nested_entity == self.player:
                    if self.world.has_component(entity, MeleeAttack):
                        melee_attack = self.world.component_for_entity(
                            entity, MeleeAttack
                        )
            ...
            if entity == self.player:
                ...
                item = self.world.component_for_entity(
                    equipped_item, item_component.Item
                )
                ...
                if self.world.has_component(equipped_item, item_component.MeleeWeapon):
                    ...
                    if self.world.has_component(
                        ...
                        for nested_entity, (
                            ...
                            if (
                                ...
                                colors = self.level_state.settings["particle_colors"][
                                    "death"
                                ]
                        ...
                        if angle_comp_func(
                            slashing_sword.angle, -150 * owner_pos.direction
                        ):
                    ...
                    if self.world.has_component(
                        equipped_item, item_component.GravityBow
                    ):
                        # Not used = no combat
                        if not item.used:
                            continue
                    ...
                    if (
                        self.world.has_component(equipped_item, item_component.Medkit)
                        and item.used
                    ):
                        owner_health = self.world.component_for_entity(
                            item.owner, Health
                        )
                        medkit = self.world.component_for_entity(
                            equipped_item, item_component.Medkit
                        )
