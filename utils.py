...
class Task:
    def __init__(self, period: int):
        ...
        self.time_instantiated = utils.time.get_ticks()
        self.last_invoked = 0
        self.period = period

    def update(self):
        if utils.time.get_ticks() - self.last_invoked > self.period:
            self.last_invoked = utils.time.get_ticks()
            return True
        return False
    ...
    def time_passed(self, time: float):
        if utils.time.get_ticks() - self.time_instantiated > time:
            return True
        return False
